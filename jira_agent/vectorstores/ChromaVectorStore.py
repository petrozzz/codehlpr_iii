...
from vectorstores.utils import xor_args
...
def _results_to_text_and_scores(results: Any) -> List[Tuple[Document, float]]:
    return [
        (text, score) for text, score in zip(
            results["documents"][0],
            results["distances"][0])
    ]


def _results_to_text(results: Any) -> List[Document]:
    return [doc for doc, _ in zip(
            results["documents"][0],
            results["distances"][0])]
...
class Chroma(VectorStore):
    ...
    def similarity_search_by_vector_with_relevance_scores(
        ...
        **kwargs: Any,
        ...
        return _results_to_text_and_scores(results)
